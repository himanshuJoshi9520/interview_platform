...
function Navbar() {
  ...
  return (
    <div className='bg-[#063a304d] flex justify-center px-4 pt-6'>
        <motion.div 
        initial={{opacity:0 , y:-40}}
        animate={{opacity:1 , y:0}}
        transition={{duration: 0.3}}
        className='w-full max-w-6xl bg-green-550 rounded-[24px] shadow-sm border border-gray-200 px-8 py-4 flex justify-between items-center relative'>
            <div className='flex items-center gap-8 cursor-pointer'>
                <div 
                  ...
                  className='flex items-center gap-3'>
                    ...
                    <h1 className='font-semibold hidden md:block text-lg'>JobPrep.AI</h1>
                </div>

                <div className='hidden md:flex items-center gap-6 border-l border-gray-200 pl-6 ml-2'>
                    <button 
                      onClick={() => navigate("/practice")}
                      className='text-slate-500 font-medium hover:text-emerald-500 transition-colors flex items-center gap-2'>
                       <span className="text-xl">💻</span> DSA Practice
                    </button>
                    <button 
            ...
            <div className='flex items-center gap-6  relative'>
                <div className='relative'>
                    <button onClick={()=>{
                        ...
                        setShowUserPopup(false)
                    }} className='flex items-center gap-2 bg-gray-10 px-4 py-2 rounded-full text-md hover:bg-green-400 transition'>
                        <BsCoin size={20}/>
                        {userData?.credits || 0}
                    </button>
