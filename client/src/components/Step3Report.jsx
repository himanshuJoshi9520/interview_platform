        ...
        {report.videoUrl && (
          <div className='lg:col-span-3'>
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className='bg-white rounded-2xl sm:rounded-3xl shadow-lg p-5 sm:p-8 flex flex-col items-center mb-2'>
              <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-4 w-full text-left">
                Interview Recording
              </h3>
              <video 
                src={`${ServerUrl}/${report.videoUrl}`} 
                controls 
                className="w-full max-w-3xl rounded-xl shadow-md border border-gray-200"
              />
              <div className="mt-6 w-full max-w-3xl flex justify-start">
                  <a href={`${ServerUrl}/${report.videoUrl}`} download={`interview_${id}.webm`} className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-semibold shadow-md transition-all flex items-center gap-2">
